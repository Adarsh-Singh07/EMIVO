...
async def process_outbox_event_search(
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            await vector_store.add_documents(
                [doc], [mock_embedding], business_id=tenant_id
            )

    except Exception as e:
        logger.error(
            f"Failed to process search {event_type} for tenant {tenant_id}: {e}"
        )
        raise
    finally:
    ...
    async def get_tenant_streams(self):
        ...
        while cursor != 0:
            cursor, partial_keys = await self.redis.scan(
                cursor, match="tenant:*:outbox:events", count=100
            )
