...
class TenantRedis:
    ...
    async def get_similar_products(self, item_id: str, limit: int = 5):
        ...
        return [
            {
                "product_id": "prod_6",
                "name": f"Similar Item to {item_id}",
                "price": 89.99,
                "score": 0.91,
...
async def get_recommendations(
    rec_type: str = Query(
        "popular",
        alias="type",
        description="Type of recommendation: popular or similar",
    ),
    item_id: str | None = Query(
        None, description="Item ID for similar recommendations"
    ),
