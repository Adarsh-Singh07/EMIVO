...
|---|---|---|
| **Frontend (Storefront)** | 🟩 Production Ready | Next.js 16 app on `elektrix.in`. Real catalog (search/filters/sort/effective pricing), cart drawer with guest→user merge, full checkout stepper (address→coupon→payment) with Razorpay Checkout.js + COD, wishlist, compare, notifications bell, account (orders/addresses/wishlist), order tracking, SEO (robots/sitemap/Product JSON-LD), `next/image`. |
...
| **Payments** | 🟩 Production Ready | Provider-agnostic `BasePaymentProvider` → real `RazorpayProvider` (live httpx) or mock. Amount validated against order.total server-side (client values ignored). Initiate creates provider order; webhook capture + Redis SETNX dedup; refund via provider API + restock; webhook failure releases reservation. On provider rejection → 503 (not raw 500). HMAC-SHA256 webhook signature verification. |
